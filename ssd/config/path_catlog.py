...
class DatasetCatalog:
    ...
    def get(name):
        if "voc" in name:
            ...
            par_root = DatasetCatalog.DATA_DIR
            if 'PAR_ROOT' in os.environ:
                par_root = os.environ['PAR_ROOT']

            attrs = DatasetCatalog.DATASETS[name]
            args = dict(
                data_dir=os.path.join(par_root, attrs["data_dir"]),
                split=attrs["split"],
            )
            return dict(factory="PARDataset", args=args)
